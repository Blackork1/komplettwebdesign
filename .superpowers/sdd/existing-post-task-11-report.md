...
## Fünfter Nachtrag: früher Cleanup-Intent und permanente Terminalkonsistenz

### Intent vor jedem vorbereitenden Zugriff

Die gemeinsame Failure Policy besitzt jetzt den einzigen allowlisteten Parser für Cleanup-Tokens. Er akzeptiert nur den generischen Reconcile-Token sowie die exakten Formen `complete`, `finish` und `fail:<allowlisteter Fehlercode>`; manipulierte Präfixe oder freie Fehlercodes werden nicht als Cleanup anerkannt. Der Produktionsworker parst diesen Intent unmittelbar nach der minimalen Revalidierungs-Payloadprüfung und damit vor Run-, Kontext-, Runtime- und Manifestzugriffen. Er trägt das normalisierte Objekt ausdrücklich bis in den Runner.

Ein transienter oder permanenter vorbereitender Fehler überschreibt einen bereits vorhandenen Cleanup-Intent nicht. Auch beim normalen Versuchslimit bleibt `complete` exakt `complete`, und `fail` behält seinen ursprünglichen Fehlercode. Der versuchsneutrale Queuepfad persistiert denselben Token und setzt `run_after`, Leasefelder, `finished_at` und den Versuchszähler weiterhin atomar. Auch ein generischer älterer Recoverytoken bleibt am ausgeschöpften Limit exakt erhalten und startet nie eine Paid Stage. Nur eine ausgeschöpfte Revalidierung ohne vorhandenen Cleanup-Intent wird weiterhin konservativ und providerfrei in den strukturierten fail-only-Pfad überführt.

### Fenced Reconcile aller Terminaloperationen

- `failRevisionRevalidation` und `completeRevisionRevalidation` werden nach jedem nicht bestätigten Abschluss – auch nach permanenten Fehlern – über denselben Revisionsfence neu gelesen. Ein bereits gespeichertes `passed` oder `failed` wird adoptiert; ein abweichender terminaler Zustand wird nicht überschrieben. Bleibt der Fence `pending` oder unlesbar, entsteht derselbe strukturierte Cleanup-Intent mit Backoff.
- `finishRun` liest nach einem nicht bestätigten Abschluss den Run erneut. Ein bereits terminaler Run wird mit seinem tatsächlichen Status und Fehlercode adoptiert. Ein weiterhin laufender oder nicht sicher lesbarer Run bleibt `finish`-Cleanup. Erst wenn dieser persistierte Run terminal ist, darf der Worker den Job terminalisieren.
- Der frühe Produktionshandler verwendet dieselbe Regel für Payload-, Kontext-, Runtime- und Manifestfehler. Permanente Fehler der frühen Revisions- oder Runpersistenz erreichen nicht mehr den generischen `failJob`-Pfad, solange der Abschluss ungeklärt ist.
- Ein reiner Cleanup lädt weder Provider noch Paid Stage, reserviert oder verändert kein Budget und erzeugt keine Lernbeobachtung. Ein `complete`-Intent ohne gefenctes, gesetteltes Reviewergebnis bleibt ausdrücklich `complete`-Cleanup. Auch mit gesetteltem Ergebnis ändern spätere Runtime-, Trusted-Context-, Quellen-, Umfangs-, Re-Audit-, Mindestscore- oder Reviewabweichungen den Intent nicht in `RETRY_EXHAUSTED` oder einen Revisionsfehler um.

### RED/GREEN-Belege

- RED: 10 von 11 gezielten Verträgen reproduzierten den verlorenen Intent vor Run-/Kontextzugriff, die fehlende Worker→Runner-Übergabe, rohe permanente Fehler aus `failRevisionRevalidation` und `finishRun`, die falsche Complete→Fail-Terminalisierung sowie fehlende commit-uncertain Adoption.
- GREEN: Dieselben 11 Kernverträge bestanden nach zentralem Parser und fenced Reconcile. Drei zusätzliche RED-Verträge für den frühen Workerpfad bestanden nach dessen Zentralisierung.
- Die unabhängige Gegenprüfung fand drei weitere Inkonsistenzen: Ein bereits terminaler Run konnte vor der Revisions-Reconciliation übernommen werden, ein unlesbarer beziehungsweise syntaktisch ungültiger nachgeladener Revisionskontext wurde fälschlich als fremder Fence behandelt und ein generischer Cleanup-Intent wurde am Versuchslimit in `RETRY_EXHAUSTED` umgedeutet. Drei zunächst rote Verträge belegen diese Fälle; nach der Korrektur bleiben Revision und Run bis zum eindeutigen Reconcile im Cleanup und der generische Intent bleibt exakt erhalten.
- Eine zweite unabhängige Gegenprüfung fand zwei verbleibende Lücken: Die Lease-Recovery übernahm terminale Revalidierungsruns noch vor dem Revisions-Reconcile, und ein `complete`-Cleanup mit gesetteltem Review konnte durch spätere lokale Abweichungen noch `failClosed` erreichen. Beide Regressionen wurden zunächst rot belegt. Die Recovery reiht jetzt jeden terminalen Revalidierungsrun versuchsneutral mit unverändertem Intent ein; normale Jobtypen übernehmen terminale Runs weiterhin direkt. Der parametrisierte Complete-Vertrag deckt Runtime, Trusted Context, Quellen, Umfang, Re-Audit, Mindestscore und Reviewergebnis kostenfrei ab.
- Die fokussierten Repository-, Worker- und Runner-Suiten bestanden anschließend mit 202 von 202 Tests. Ein eigener Runtime-/Manifestvertrag belegt zusätzlich den unveränderten kostenfreien `complete`-Intent vor beiden Prüfungen.
- Der echte PostgreSQL-Test führt zwei aufeinanderfolgende `complete`-Reschedules am normalen Versuchslimit aus. Beide bewahren exakt `last_error`, geben den begonnenen Claimversuch zurück, setzen die angegebene Retryzeit und lassen den Job nicht terminal. Weitere echte Recoveryfälle belegen dieselbe Eigenschaft für den generischen Cleanup-Token bei ausgeschöpften Versuchen sowie für einen bereits terminalen Revalidierungsrun, dessen Job bis zum Revisions-Reconcile `queued` bleibt. Die gesamte PostgreSQL-Suite bestand mit 12 von 12 Tests.

### Abschließende Verifikation

- Fokussierte Repository-, Worker- und Runner-Suiten: 202 bestanden, 0 fehlgeschlagen, 0 übersprungen.
- Echter PostgreSQL-Lauf mit explizit freigegebener lokaler Testdatenbank: 12 bestanden, 0 fehlgeschlagen, 0 übersprungen.
- Vollständige Suite mit lokalem, nicht verwendetem Dummywert `OPENAI_API_KEY=test-key`: 1.816 bestanden, 0 fehlgeschlagen, 12 geschützte PostgreSQL-Opt-in-Tests übersprungen; 1.828 Tests insgesamt.
- `npm run build`: erfolgreich; 41 CSS-Quelldateien verarbeitet, Manifest unverändert.
- Syntaxprüfungen und `git diff --check`: ohne Befund.
- Externe OpenAI-, Cloudinary-, SMTP- oder Produktionsaufrufe wurden nicht ausgeführt.
