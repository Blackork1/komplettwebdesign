...
const MIGRATIONS = [
  ...
  './migrations/016_update_package_meta_descriptions.sql'
];
...
if (currentFile === entryFile) {
  runContentAgentMigration()
    .then(async () => {
      console.log('Content-Agent-Migration 002 bis 016 erfolgreich.');
      await pool.end();
    })
    .catch(async (error) => {
      console.error('Content-Agent-Migration 002 bis 016 fehlgeschlagen:', error.message);
      await pool.end();
      process.exitCode = 1;
    });
}
