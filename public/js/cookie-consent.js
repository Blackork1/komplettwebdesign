    ...
    if (banner) {
        function setConsent(analytics, marketing) {
            fetch('/api/consent', {
                ...
                .then(r => {
                    if (!r.ok) {
                        // Wenn Server einen Fehlerstatus liefert: als Text auslesen und anzeigen
                        return r.text().then(text => {
                            console.error('Consent-API Error:', text);
                            throw new Error('Consent API responded with status ' + r.status);
                        });
                    }
                    return r.json();
                })
                .then(json => {
                    if (json.success) {
                        ...
                        console.error('Consent not saved:', json.message || json);
                    }
                })
                .catch(err => {
                    console.error('Fetch error in setConsent:', err);
                });
        }
